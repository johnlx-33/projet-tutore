...
Nous avons tout d'abord pris connaissance du théorème, puis nous avons chercher et appris plusieurs facon de corriger le théorème des restes chinois lors de la présence d'erreurs.
...
Pourtant il se révéle trés pratique dans des calcules à grandes échelles trés rapidement en minimisant les possibilité d'erreur de calcul.
Ainsi qu'en cryptologie oû la possibilité de faire des calculs sur de grand nombres avec des valeurs discretes permettent d'assuré une certaine sécurité si toutefois une partie des données fuitez.
...
Maintenant que nous avons présenté les algorithmes principaux du théorème des restes chinois, voici quelque fonction que nous pourrions qualifié de segondaire au premier abord, mais très utile dans certains cas. \newline
En premier, nous avons fait un algorithmes s'intitulant générateur de cas, qui prend en argument 2 nombres et qui génère les listes des nombres premier entre eux, et la liste des reste modulo les $p_i$. C'est en
quelque sorte la fonction inverse du théorème des restes chinois. 

\begin{algorithm}
    \caption{générateur de cas}
    \begin{algorithmic}
        \REQUIRE $n>k$
...
Par exemple générateur de cas $(30030,61)$ renvoie les listes $P=[2,3,5,7,11,13]$ et la liste (àcompléter)!!!! \newline

Nous pouvons remarquer que dans la fonction au dessus, nous utilisons la fonction nombre facteur premier qui prend en paramètre un nombre n et renvoie la liste des ses facteurs premier. 
...
Dans les algorithmes que vous verrez dans la suite de notre travaille, vous pourrez trouver nos calculs de borne. Cependant, ces calculs sont propres à chaque algorithme,
c'est à dire que la borne pour un meme nombre et un meme nombre d'erreur peut varier en focntion des algorithmes utilisé. \newline
Mais, on peut calculer mathématiquement la borne maximale. \newline
Soit t le nombres d'erreurs et 
Cela signifie que aprsè cette borne, la correction d'erreurs marche parfois, ce n'est donc plus une valeur sur à cent pour cent, alors que avant cette borne, oui.
\newline
La question que nous sommes à présent en droit de se poser est, es qu'il exite une borne tel que l'on ne puisse corriger aucun nombres supèrieur à cette borne. Et la réponse est oui.
Partons d'un n-uplet de départ $x=(x_1, x_2, ..., x_n)$. On pose également, dans le n-uplet reçu, $y$ le resultat possible avec t erreurs. Avec biensur, le cardinal des n-uplets reçu qui est infèrieur à la sommes des $p_i$. Dans les n-uplets reçu, il y a un nombre $D$ de résultat possibles. Nous cherchons donc à calculer $D$, qui vaut: \newline
...
\begin{appendices}
    \begin{algorithm}
        \caption{brute force détection 1 erreur}
        \begin{algorithmic}
            \REQUIRE $len(P)=len(N)$
            \STATE $L1=[]$
            \FOR {i in range(len(N))}
